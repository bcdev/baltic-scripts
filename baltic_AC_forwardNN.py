...
def write_BalticP_AC_Product(product, baltic__product_path, sensor, data_dict, singleBand_dict=None):
    ...
    # Set auto grouping
    autoGroupingString = ':'.join(data_dict.keys())
    ...
    if not singleBand_dict is None:
        for key in singleBand_dict.keys():
            data = singleBand_dict[key].get('data')
            if not data is None:
...
def baltic_AC_forwardNN(scene_path='', filename='', outpath='', sensor='', subset=None, addName = ''):
    ...
    data_dict = {
            ...
            'rho_rc':{'data': rho_rc},
            'rho_ag':{'data': rho_ag},
            'rho_ag_mod':{'data': rho_ag_mod},
            'td':{'data': td},
            'rho_w':{'data': rho_w},
            'rho_wmod':{'data': rho_wmod},
            'rho_wn': {'data': None},
            'unc_rhow': {'data': None}
            }
    ...
    singleBand_dict =  {
            'log_apig':  {'data': iop[:,0]},
            'log_adet':  {'data': iop[:,1]},
            'log_agelb': {'data': iop[:,2]},
            'log_bpart': {'data': iop[:,3]},
            'log_bwit':  {'data': iop[:,4]}
            }

    write_BalticP_AC_Product(product, baltic__product_path, sensor, data_dict, singleBand_dict)

...
def apply_forwardNN_IOP_to_rhow_example():
	...
	test_rhow_fromTOA = np.array((0.009959362, 0.01134242, 0.014025537, 0.01487543, 0.010059207, 0.005442682,
								  0.0010344568, 5.945379E-4, 5.734044E-4,
...
def apply_forwardNN_IOP_to_rhow(iop, sun_zenith, view_zenith, diff_azimuth, sensor, T=15, S=35, nn='2016'):
	...
	if nn == 'alt':
		nnFilePath = "E:\software\olci_netze_20171221\iop_rw\\77x77x77_1798.8.net"
...
def write_BalticP_AC_Product(product, baltic__product_path, sensor, data_dict, singleBand_dict=None, basedonL1=True):
...
def L2iop_Reader(product, sensor, c2rcc_version='2016'):
	input_label = []
	if sensor == 'OLCI':
		if c2rcc_version == '2016' or c2rcc_version== 'alt':
			input_label = ['iop_apig', 'iop_adet', 'iop_agelb', 'iop_bpart', 'iop_bwit']
		elif c2rcc_version == '2019':
			input_label = ['iop_apig', 'iop_adet', 'iop_agelb', 'iop_bpart', 'iop_bwit']
...
def L2Reflectance_Reader(product, sensor, band_group='rhow'):
	input_label = []
	if sensor == 'OLCI':
		if band_group == 'rhow':
			input_label = ["rhow_" + str(i + 1) for i in range(12)]
...
def check_valid_pixel_expression_L2(product, sensor):
	...
	if sensor == 'OLCI':
		...
		#Rhow_OOR	4	uint32		One of the inputs to the IOP retrieval neural net is out of training range
		#Cloud_risk	8	uint32		High downwelling transmission is indicating cloudy conditions
		#Iop_OOR	16	uint32		One of the IOPs is out of range
		c2rcc_flags = np.zeros(width * height, dtype='uint32')
		product.getBand('c2rcc_flags').readPixels(0, 0, width, height, c2rcc_flags)

		c2rcc_flags = (np.bitwise_and(c2rcc_flags, 2 ** 2) == 2 ** 2)| \
				   (np.bitwise_and(c2rcc_flags, 2 ** 3) == 2 ** 3) | \
				   (np.bitwise_and(c2rcc_flags, 2 ** 4) == 2 ** 4)

		valid_pixel_flag = np.logical_and(valid_pixel_flag, np.logical_not(c2rcc_flags))


	return valid_pixel_flag


def chi_square_spectral_test(x, model, normalise=True, colN_560=5):

	N = x.shape[0]

	if normalise:
		###
		# Normalisation is needed to test on the similarity of spectral shape only.
		# normalise on band 560nm

		for i in range(x.shape[1]):
			x[:, i] = x[:, i] / x[:, colN_560]
			model[:, i] = model[:, i] / model[:, colN_560]

		x = np.delete(x, np.s_[colN_560], axis=1)
		model = np.delete(model, np.s_[colN_560], axis=1)

	M = x.shape[1]

	# Goodness of fit for all data points and wavelengths.
	# M=11 wavelengths (from 12 OLCI bands) remain, N = number of data points
	# M = M * N
	# chisquare_all = np.nansum(np.power(model-x, 2.) / x)
	##test_all = qchisq(p=0.95, df=M-1)

	# chi-square value for each spectrum individually
	chi_square_array =  np.sum(np.power(model-x, 2.) / x, axis=1)

	### Further analysis as done in RR-OCCCI phase 1 and 2. Not necessary.
	# dann wird die Anzahl an Fällen nachher gezählt, für die der Test erfüllt ist.
	# Anzahl der freiheitsgrade: df= 11 Wellenlängen - 1 = 10
	# Signifikanz-Niveau alpha=95% . chi-square (1-alpha,df)
	# Ähnlichkeit zwischen insitu Spektrum und satellite spektrum wird abgelehnt, wenn das ermittelte chi.square größer ist als die Prüfgröße qchisq zu alpha= 95% und df=3
	# Anzahl der Wellenlängen M2
	# M2 = model.shape[1]
	# Signifikanz-Niveau 95%.
	#test = qchisq(p=0.95,df=M2-1)
	#ID < - chi.square < test & chi.square >= 0
	#mean.chi2_95 < - mean(chi.square[ID])

	return chi_square_array


def mean_absolute_error_perSpectrum(x, model, relative = False):
	if relative:
		out = np.sum(np.abs(model-x) / x, axis=1)
	else:
		out = np.mean(np.abs(x-model), axis=1)

	return out



def test_quality_forwardNN_scene(scene_path='', filename='', outpath='', sensor='', c2rcc_version = '2016'):
	###
	...
	###
	# Read L2 product c2rcc - read only 5 iops.
	# returns: numpy array with shape(pixels, iops).
	# iops: log_apig, log_adet, log a_gelb, log_bpart, log_bwit -> input for NN!
	###
	iop = L2iop_Reader(product, sensor=sensor, c2rcc_version=c2rcc_version)
	iop = np.log(iop)

	###
	# Read angles from product.
	# returns: OAA, OZA, SZA, SAA
	###
	oaa, oza, sza, saa = angle_Reader(product, sensor=sensor)

	diff_azim = calculate_diff_azim(oaa, saa)

	###
	# Read L2 product c2rcc - read rhow.
	# returns: numpy array with shape(pixels, iops).
	refl = L2Reflectance_Reader(product, sensor=sensor)

	###
	# classification of pixels
	# returns: boolean array (pixels).
	###
	valid_L2 = check_valid_pixel_expression_L2(product, sensor)

	for i in range(refl.shape[1]):
		refl[np.logical_not(valid_L2), i] = np.nan


	###
	# apply forwardNN: iop + angles -> rhow
	###
	rhow_mod = apply_forwardNN_IOP_to_rhow(iop, sza, oza, diff_azim, sensor, nn=c2rcc_version)

	###
	# calculate chi square for each pixel.
	# Normalisation is needed to test on the similarity of spectral shape only.
	###
	chiS = chi_square_spectral_test(x=np.copy(refl), model=np.copy(rhow_mod))
	#chiS = chi_square_spectral_test(x=np.copy(refl), model=np.copy(rhow_mod), normalise=False)

	###
	# calculate MAE for each pixel
	# log-transform needed? to give equal weight to differences in small values?
	# mean absolute relative error...
	###
	MAE = mean_absolute_error_perSpectrum(x=np.copy(refl), model=np.copy(rhow_mod), relative=False)
	MAErel = mean_absolute_error_perSpectrum(x=np.copy(refl), model=np.copy(rhow_mod), relative=True)

	###
	# Writing a product
	# input: data_dict holds the band
	###
	baltic__product_path = outpath + 'test_forwardNN_' + filename
	data_dict = {
		'rhow': {
			'data': refl
		}
	...
	write_BalticP_AC_Product(product, baltic__product_path, sensor, data_dict, singleBand_dict, basedonL1=False)
	#write_BalticP_AC_Product(product, baltic__product_path, sensor, data_dict, basedonL1=False)
...
def main(args=sys.argv[1:]):
	...
	path = 'E:\Documents\projects\Baltic+\WP3_AC\\test_data\\'
	outpath = path

	#fnames = os.listdir(path)
	#fnames = [fn for fn in fnames if '.dim' in fn]  # OLCI
	### Subsetting mit pixelGeoCoding on.
	#fnames = ['subset_xfdumanifest_C2RCC.dim', 'subset_xfdumanifest_C2RCC_v2019.dim', 'subset_xfdumanifest_C2RCCAlt.dim']
	### Subsetting mit pixelGeoCoding off.
	fnames = ['subset_S3A_OL_1_EFR___20190409_nopixelgeocod_C2RCC2016.dim', 'subset_S3A_OL_1_EFR___20190409_nopixelgeocod_C2RCC2019.dim', 'subset_S3A_OL_1_EFR___20190409_nopixelgeocod_C2RCCAlt.dim']
	c2rcc_version = ['2016', '2019', 'alt']

	print(len(fnames))

	for cv, fn in zip(c2rcc_version[:], fnames[:]):
		print(fn, cv )
		#apply_NN_to_scene(scene_path=path, filename=fn, outpath=outpath, sensor=sensor)
		#test_quality_forwardNN_scene(scene_path=path, filename=fn, outpath=outpath, sensor=sensor)
		test_quality_forwardNN_scene(scene_path=path, filename=fn, outpath=outpath, sensor=sensor, c2rcc_version=cv)
